...
fun PlayerSheets(
  ...
  when (sheetShown) {
    ...
    Sheets.SubtitleSearch -> {
      val videoTitle = viewModel.mediaTitle.composeCollectAsState().value.takeIf { it.isNotBlank() }
        ?: viewModel.getCurrentMediaTitle()
      val subtitlesPreferences = org.koin.compose.koinInject<app.marlboroadvance.mpvex.preferences.SubtitlesPreferences>()
      val preferredLanguage = subtitlesPreferences.preferredLanguages.get().split(",").firstOrNull()?.trim()
      SubtitleSearchSheet(
          initialQuery = videoTitle.substringBeforeLast("."), // Remove extension for cleaner search
